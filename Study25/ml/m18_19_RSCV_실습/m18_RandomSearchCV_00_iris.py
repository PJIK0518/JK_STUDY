...
pd.DataFrame(model.cv_results_).sort_values('rank_test_score', ascending=True).to_csv(path + f'm18_{saveN}_GSCV_results.csv')
...
import joblib
print('훈련 소요시간 :', time.time() - S)
joblib.dump(model.best_estimator_, path + f'm18__{saveNum}_best_model.joblib')
print(saveNum, '저장완료')