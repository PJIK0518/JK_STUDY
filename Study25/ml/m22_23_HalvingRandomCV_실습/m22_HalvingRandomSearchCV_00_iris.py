...
model = HalvingRandomSearchCV(XGBClassifier(),       
                    ...
                    #  n_iter=50,     # 반복해보는 횟수 : GridSearch 에서 파샌된 애라서 없음
                     ...
                     min_resources = 40,    # 최초 사용 데이터 양   factor * min_resource가 max_resource를 넘어가면 훈련 중단
...
pd.DataFrame(model.cv_results_).sort_values('rank_test_score', ascending=True).to_csv(path + f'm22_{saveN}_GSCV_results.csv')
