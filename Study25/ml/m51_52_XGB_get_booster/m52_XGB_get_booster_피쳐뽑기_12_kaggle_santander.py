...
gain_list = [gain.get(f'f{i}', 0) / total for i in range(x_trn.shape[1])]
thresholds = np.sort(gain_list)

feature_names = np.array(x.columns)  # ✅ 반드시 feature_names를 x 기준으로

# 4. Feature selection 루프
BEST_col = []
BEST_drp = []
BEST_scr = 0
BEST_trn = x_trn
...
for i in thresholds:
    ...
    if select_x_trn.shape[1] == 0:
        print(f'DONE at threshold {i:.4f} (0 features)')
        continue
    ...
    D_feature = feature_names[~Columns]
    ...
    print(f'Threshold = {i:.4f} / n = {select_x_trn.shape[1]:2d} / ACC = {score*100:.3f}%')
    print(C_feature)
    print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')

print(f'최적 컬럼 : {BEST_trn.shape[1]} 개\n', BEST_col)
print(f'삭제 컬럼 : {x_trn.shape[1] - BEST_trn.shape[1]} 개\n', BEST_drp)
print(f'최고 점수 : {BEST_scr*100:.3f}%')
