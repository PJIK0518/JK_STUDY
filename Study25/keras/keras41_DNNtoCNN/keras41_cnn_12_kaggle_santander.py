...
# model.add(Conv2D(10, 1, padding='same', input_shape = (20,10,10)))
# model.add(Conv2D(10, 1))
# model.add(Flatten())
x_trn = x_trn.reshape(-1,10,10,2)
x_tst = x_tst.reshape(-1,10,10,2)
x_val = x_val.reshape(-1,10,10,2)
...
def LT(a,b,c,d,e,DO):
        model = Sequential()
        model.add(Conv2D(50, 2, padding='same', input_shape = (10,10,2), activation='relu'))
        ...
        model.add(BatchNormalization())
        model.add(Conv2D(10, 2, activation='relu'))
        model.add(Dropout(DO))
