...
from tensorflow.python.keras.layers import Dense, Dropout, BatchNormalization, MaxPool2D
...
x_trn = np.array(x_trn).reshape(-1,3,3,1)
x_tst = np.array(x_tst).reshape(-1,3,3,1)
# model.add(Conv2D(10, 1, padding='same', input_shape = (shape)))
...
model = Sequential()
model.add(Conv1D(10, 2, padding='same', input_shape = (3,3,1), activation='relu'))
...
'''
GPU 있다!!!
소요시간 : 21.05644655227661

GPU 없다...
소요시간 : 4.870558023452759
