...
#0. 준비
##########################################################################
from tensorflow.keras.layers import Dense, Conv1D, Lambda, Dropout, BatchNormalization, LSTM, GRU, SimpleRNN, Bidirectional, MaxPool1D, Flatten
...
x = pd.read_csv(path_D + 'x_trn_0620_8.csv', index_col=False)
y = pd.read_csv(path_D + 'y_trn_0620_8.csv', index_col=False)
...
##########################################################################
### 불러오기
# model = load_model(path_S + 'jena_0623_0.h5')
#####################################
# model.summary()
# exit()
...
model.add(Conv1D(12,5, input_shape = (x_trn.shape[1],x_trn.shape[2]),
                 activation='sigmoid'))
model.add(MaxPool1D(5))
model.add(Dropout(0.5))

model.add(BatchNormalization())
model.add(Conv1D(20,3, activation='sigmoid'))
...
'''
rmse : 74.29011305308136

rmse : 55.75645292325104
'''
...
### 가중치 불러오기
# model.load_weights(path_W + '0620_5.h5')
