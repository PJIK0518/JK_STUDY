...
#0. 준비
##########################################################################
from tensorflow.keras.layers import Dense, Lambda, Dropout, BatchNormalization, LSTM, GRU, SimpleRNN, Bidirectional
...
##########################################################################
### 불러오기
model = load_model(path_S + 'jena_0620_5.h5')
#####################################
model.summary()
exit()
...
model.add(GRU(144, input_shape = (x_trn.shape[1],x_trn.shape[2]),
               activation='sigmoid', return_sequences=True))
model.add(Dropout(0.2))

model.add(BatchNormalization())
model.add(GRU(144, activation='sigmoid'))
# model.add(Dropout(0.2))
...
y_tru = pd.read_csv('C:/Study25/_data/kaggle/jena/sub/' + 'y_prd_0620_8.csv', index_col=False)

x_prd = np.array(x_prd)
y_tru = np.array(y_tru)

x_prd = x_prd.reshape(-1,144,13)
