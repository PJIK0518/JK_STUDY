...
x_trn = np.array(x_trn).reshape(-1,5,2,1)
x_tst = np.array(x_tst).reshape(-1,5,2,1)
...
model = Sequential()
model.add(Conv1D(80, 2, padding='same', input_shape = (5,2,1), activation = 'relu'))
model.add(Dropout(0.2))

model.add(BatchNormalization())
model.add(Conv2D(50, 2, padding='same', activation = 'relu'))
model.add(Dropout(0.2))
