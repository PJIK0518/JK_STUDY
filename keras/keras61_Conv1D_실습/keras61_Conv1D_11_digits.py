...
x_trn = np.array(x_trn).reshape(-1,8,4,2)
x_tst = np.array(x_tst).reshape(-1,8,4,2)
# model.add(Conv2D(10, 1, padding='same', input_shape = (8,4,2)))
...
def f(a,b,c):
        model = Sequential()
        model.add(Conv1D(20, 2, padding='same', input_shape = (8,4,2)))
