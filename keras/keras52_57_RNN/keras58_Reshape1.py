...
#2. 모델 구성
model = Sequential()
model.add(Dense(100, input_shape = (28,28)))                   # Dense의 경우 2차원이상을 받아드리면 차원은 유지하면서 송출
# ValueError: Input 0 of layer "conv2d" is incompatible with the layer: expected min_ndim=4, found ndim=3.
#          >> Full shape received: (None, 28, 100) : shape도 dim도 안 맞음
#          >> conv2d는 4차원을 원하는데 Dense는 3차원을 받았을때 3차원을 줘서 불가능
#          >> conv2d의 input_shape와 맞추려면 Dense layer의 unit을 28로 맞출 수 있게 조절 필요
model.add(Reshape(target_shape=(28,10,10))) # (H, W, C) 의 형태로 구성됨
#          >> OutPutShape :(N, 28, 10,  10)
#          >> 통상적으로 기존 이미지 파일을 전처리 과정에서 shape을 맞추고 하는 것이 성능이 더 우수
#          >> 모델 내에서 shape을 맞추기 위해서 사용                             
