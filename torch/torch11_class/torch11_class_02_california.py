...
""" print(x_trn.size()) torch.Size([14447, 8]) """
""" print(y_trn.size()) torch.Size([14447, 1]) """
...
# class : 괄호 안에 있는 것 끌어다 쓰겠다
class Model(nn.Module):
    def __init__(self, input_dim, output_dim):      # 클래스 안의 함수 : Method (in nn.Module)
                                                    # default 로 넣어줘야하는 값 (input_dim, output_dim)
        super().__init__()                          # 받아들이는 녀석들을 초기화
        # = super(Model, self).__init__()           # super 내부에는 어떤 클래스를 사용할지 기입
        
        ### 모델 정의 ###                           # 모델에 사용할 레이어를 정의 (순서상관 X)
    ...
    def forward(self, x):                           # 정의된 layer를 불러와서 모델 구성              
        x = self.linear1(x)
        x = self.relu(x)
        x = self.drop(x)
