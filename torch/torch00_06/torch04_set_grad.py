...
''' in tensorflow...
    ...
    # with torch.no_grad():               # 기울기 계산 및 가중치 갱신을 하지 않겠다
...
def evaluate(model, criterion, x, y):
    ...
    torch.set_grad_enabled(False)       # 기울기를 계산하지 않는다 : 가중치 갱신을 하지 않는다
    y_prd = model(x)                    # 예측
    F_lss = criterion(y, y_prd)         # 오차 계산
    torch.set_grad_enabled(True)        # fit 상태로 모델 복구
...
x_prd = (torch.Tensor([[n, m]]).to(DEVICE) - mean) / std

reslt = model(x_prd)

print('Final_loss :', F_lss)
print(f'Predict{[n, m]} :', reslt.item())
